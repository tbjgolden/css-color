...
suite('rgb', () => {
  ...
  for (let i = 0; i < numPermutations; i++) {
    ...
    const runTest = (alphaIsPercentage) => {
      let channelValues = usesPercentages
        ? ['50%', '60%', '25%']
        : ['127', '153', '64']
...
suite('hsl', () => {
  ...
  for (let i = 0; i < numPermutations; i++) {
    ...
    const [functionNameEndsInA, hasAlpha, hasCommas] = variantValues
    ...
    const runTest = (alphaIsPercentage) => {
      ...
      chosen[chosen.length - 1] =
        hueUnits === '' ? 'no hue unit' : `hue unit is ${hueUnits}`
      if (hasAlpha) {
        chosen.push(
          alphaIsPercentage ? 'alpha is a percentage' : 'alpha is a float'
        )
