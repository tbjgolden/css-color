  ...
  for (let i = 0; i < iterations; i++) {
    ...
    hexColors.push(
      '#' +
        (
          '0'.repeat(hexLen) +
          ((Math.random() * 2 ** (hexLen * 4)) >>> 0).toString(16)
        ).slice(-hexLen)
    )
  }
...
const benchRgba = () => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    rgbaColors.push(
      `rgb${Math.random() > 0.5 ? 'a' : ''}(${randomColorValue(
        usePercentages
      )}${comma} ${randomColorValue(usePercentages)}${comma} ${randomColorValue(
        usePercentages
      )}${hasAlpha ? (comma ? comma + ' ' : ' / ') : ''}${
        hasAlpha ? alpha : ''
      })`
    )
  }
...
const benchHsla = () => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    colors.push(
      `hsl${
        Math.random() > 0.5 ? 'a' : ''
      }(${randomHue()}${comma} ${randomColorValue()}${comma} ${randomColorValue()}${
        hasAlpha ? (comma ? comma + ' ' : ' / ') : ''
      }${hasAlpha ? alpha : ''})`
    )
  }
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const alpha =
      Math.random() > 0.5
        ...
        : Math.random().toFixed(2)
    colors.push(
      `hwb(${randomHue()} ${randomColorValue()} ${randomColorValue()}${
        hasAlpha ? ' / ' : ''
      }${hasAlpha ? alpha : ''})`
    )
  }
